...
def create_one_batch(x: List[List[str]],
                     word2id: Union[Dict[str, int]],
                     char2id: Union[Dict[str, int]],
                     config: Dict,
                     oov: str = '<oov>',
                     pad: str = '<pad>',
                     sort: bool = True
    ...
    Note: sort, a boolean value, specifies whether sorting the sentences by their lengths.
...
class Model(nn.Module):
    ...
    def forward(self,
                word_inp: List[torch.LongTensor],
                chars_package: List[torch.LongTensor],
                mask_package: List[List[torch.LongTensor]]
                ):
